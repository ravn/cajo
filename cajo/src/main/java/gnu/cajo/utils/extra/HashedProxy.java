...
/**
 * This class is used to hash a ZippedProxy. Primarily it is intended to simply
 * and quickly convert the serialized proxy representation in to, and out of
 * plaintext, for transmission over the network. It should be noted that the
 * default hashing algorithm is <u>not</u> cryptographically strong. <i>(The
 * author lives in the United States and doesn't want to go to jail)</i> It's
 * purpose is to provide a simple example encryption, which could be sufficient
 * for many purposes. It can always be subclassed, to employ much more
 ...
 * encrypted streams in that each object can employ different cryptography. If
 * the client and the server both have the derived class in their local
...
public class HashedProxy extends gnu.cajo.utils.ZippedProxy {
	...
	/**
	 * A flag to indicate if the payload has been hashed. To reduce overhead, the
	 * payload is hashed only once per session.
	 */
	protected transient boolean hashed;

	/**
	 * A symetric hash algorithm, to convolve the array in to and out of plain text.
	 * It is invoked once at the sending VM, then once at the receiving VM.
	 */
	 ...
	 *
	 * @param proxy
	 *            The client proxy object, to be hashed at the server, and unhashed
	 *            at the client.
	 */
