 ...
 * reference with it. Ideally, many instances of the servers would be running on
 * the network. To keep the server memory footprint manageable, only one object
 * is allowed per virtual machine IP address, any subsequent entries overwrite
 * the previous. It also runs a lightwieght task, which periodically runs
 * through the registry, purging object references that have become invalid. To
 * encourage spontaneous internetworking, the registry will announce itself
 * hourly, on the cajo hailing frequency, and listen on it, for other reference
 * announcements; which it will automatically register.
 *
...
public final class Registry {
	...
	/**
	 * This method is called solely by this registry's Multicast member object, to
	 * register objects of remote server announcements.
	 */
	...
	/**
	 * This method is called by remote VMs, to request a copy of the remote object
	 * reference registry.
	 *
	 * @return A hashtable containing all the references, keyed by their server
	 *         addresses
	 */
	...
	/**
	 * Always a good idea; this method describes how to use the registry object, and
	 * what its features are. Not unlike these comments themselves.
	 *
	...
	/**
	 * This method is used to start up the registry server. It will announce its
	 * startup on the hailing frequency, to allow linkage to other registries,
	 * before starting to listen on it, for new announcements.
	 * <p>
	 * <i>Note:</i> to build this application, it will also require an RMI stub
	 * class for Remote. This means you must also run the following instruction:
	 * <p>
	 * <blockquote>rmic -v1.2 gnu.cajo.invoke.Remote</blockquote>
	 *
	 * @param args
	 *            If the server is behind a NAT router, the only argument should be
	 *            the network address used outside the router. Otherwise provide
	 *            nothing, and it will use the default local address.
	 */
