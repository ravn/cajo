...
/**
 * This class is used to send server item callbacks to a firewalled client. A
 ...
 * ItemProxy} object. The server item invokes methods on this client proxy which
 * result in an immediate callback invocation on the client.
 * <p>
 * Instances of ClientProxy can in turn be passed into a
 * {@link TransparentItemProxy TransparentItemProxy} via its get method, to a
 * address the firewalled client by interface. (either synchronously <i>or</i>
 * asynchronously)
 * <p>
 ...
 * remote client object. A practical usage <a
...
public final class ClientProxy implements Invoke {
	...
	/**
	 * This is the longest value, in milliseconds, that the server will wait for a
	 * client invocation to execute before it aborts it. It is set by default to
	 * 5000 (5 seconds). Depending on the type of functionality being performed in
	 * the client methods, this time may require adjusting. When it is changed, the
	 * new value will apply for all subsequent calls.
	 */
	public int timeout = 5000;
	/**
	 * This is the remoted reference to the server's ClientProxy. It is passed back
	 * to the client, to be used int the {@link ItemProxy ItemProxy} constructor, to
	 * create a firewall traversing asynchronous callback link. In order to work,
	 * logically, it can only be passed to one remote client.
	 */
	public final Remote remoteThis;

	/**
	 * A server creates this object, then provides the remote reference member
	 * remoteThis to the client. This creates the first half of the bridge, the
	 * {@link ItemProxy ItemProxy} class completes the second half.
	 *
	...
	/**
	 * This method abruptly terminates the ClientProxy link to the server. The
	 * client can detect the detachment, if it wishes, by implementing a similar
	 * cutOff method. The client may also remotely invoke this method, if it wishes
	 * to sever its link to the server. Any subsequent invocations by the server
	 * will result in client timeout exceptions. This method also calls notify(), in
	 * case there was an invocation in process when this happened.
	 * <i><u>Note</u>:</i> it would not make sense to call this method more than
	 * once, ever, for a given object instance.
	 *
	 * @throws NoSuchObjectException
	 *             Should this method ever be called more than once, on the same
	 *             object.
	 */
	 ...
	 * Initially a remote {@link ItemProxy ItemProxy} calls this method to have its
	 * calling thread blocked until the server item needs to make an asynchronous
	 * callback. Secondly, the server item will also invoke this method, and will
	 * have its thread blocked, until the resulting data, or exception, is returned
	 * from the firewalled client, via its ItemProxy.
	 *
	 * @param method
	 *            The name of the method on the firewalled remote client to be
	 ...
	 *             If the client is not listening, or if the callback timeout has
	 *             expired.
	 * @throws RemoteException
	...
	public synchronized Object invoke(String method, Object args) throws Exception {
		if (method == null) { // client callback response thread
			...
			if (!connected)
				wait(timeout); // delay for initial client connect
			if (!done)
