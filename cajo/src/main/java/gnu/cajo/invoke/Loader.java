 ...
 * this graphical widget will be created. It allows the Java Virtual Machine to
 * host multiple proxy items, from remote servers. The user gets a small AWT
 * dialog box; into which to type the host, port, and item name. It will take
 * care of loading the proxy, and displaying its GUI, if it has one. If the
 * graphical proxy component implements WindowListener, it will be added as a
 * listener to its display frame automatically, before it is made visible. Note:
 * closing the dialog will intentionally terminate the JVM, and consequently all
 * of its loaded proxies. If the returned proxy is an AWT component, it will be
 * displayed in a double buffered frame, if it is a JComponent, its
 * setDoubleBuffered(true) will be called, before it is displayed.
 *
...
final class Loader extends Frame implements WindowListener, ActionListener {
	...
	Loader() {
		...
		main = true;
		setLayout(null);
		final int WIDTH = 250, HEIGHT = 200, ROW1 = HEIGHT / 6, ROW2 = 2 * ROW1, ROW3 = 3 * ROW1, ROW4 = 4 * ROW1;
