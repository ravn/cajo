...
public final class Display extends Widget {
	...
	public void setBounds(int x, int y, int width, int height) {
		x += leftAligned ? 0 : rightAligned ? width - columns * charWidth - 3 : (width - columns * charWidth) / 2 - 2;
		y += topAligned ? 0 : bottomAligned ? height - rows * charHeight - 3 : (height - rows * charHeight) / 2 - 2;
	...
	public void clear() {
		...
		for (int i = 1; i < limit; i += i)
			System.arraycopy(buffer, 0, buffer, i, ((limit - i) < i) ? (limit - i) : i);
	}
	...
	public void display(String s, boolean invert) {
		display(s.toCharArray(), 0, s.length(), invert);
	}

	public void display(char data[], int offset, int len, boolean invert) {
		for (int ix = 0; ix < len; ix++)
			display(data[offset + ix], invert);
	}
	...
	public void display(char character, boolean invert) {
		...
		case '\b': // backspace
			if (index > 0) {
				buffer[--index] = (byte) (invert ? 0xa0 : 0x20);
				repaint((index % columns) * charWidth + 2, (index / columns) * charHeight + 2, charWidth, charHeight);
			}
			break;
		...
		for (int iy = 0; iy < rows; iy++) {
			for (int ix = 0; ix < columns; ix++) {
				...
				if ((buffer[chpos] & 0x80) != 0) {
					do
						line[len++] = (byte) (buffer[chpos++] & 0x7f);
