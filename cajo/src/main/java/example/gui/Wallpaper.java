...
public final class Wallpaper extends Widget implements ImageObserver {
	...
	public Wallpaper(String file, boolean tiled) throws IOException {
		...
		this.tiled = tiled;
		if (file != null) {
			InputStream is = new BufferedInputStream(getClass().getResourceAsStream(file));
			byte bytes[] = new byte[is.available()];
			is.read(bytes);
			is.close();
			Toolkit tk = getToolkit();
	...
	public boolean imageUpdate(Image i, int f, int x, int y, int w, int h) {
		if (iw == 0 && (f & ImageObserver.WIDTH + ImageObserver.HEIGHT) == ImageObserver.WIDTH + ImageObserver.HEIGHT) {
			iw = w;
			ih = h;
			calcOffsets();
		}
			...
			if (tiled) {
				...
				for (int wc, xpos = 0; xpos < width; xpos += iw) {
					...
					for (int hc, ypos = 0; ypos < height; ypos += ih) {
						...
						g.drawImage(photo, xpos, ypos, xpos + wc, ypos + hc, 0, 0, wc, hc, this);
					}
				}
			} else
